...
arow(6,  "SEEK base, FY2027", 4626, CUR, "Enacted: 2026 Ky. Acts ch. 168 (HB 500), p. 20, archived in build/; an interim v5.0 draft wrongly moved this to $4,636; corrected here")
...
put(df, "A13", "All-funds dollars coded to org 090, fiscal 2026 MUNIS Cost by ORG"); put(df, "B13", 1285310, BLUE, CUR)
...
put(df, "A30", "Carried funding of the 128 students on the official 2024-25 roster: grade counts (K-5) x years to grade 12"); 
...
put(c, "A20", "NET RECURRING GENERAL FUND SAVING (LEGACY single-point scenario on superseded inputs; the published model is the 972-scenario grid at row 37, median a $427,087 LOSS)", bold=True)
...
put(c, "A31", "Each departing student removes approximately the SEEK base guarantee, and more with the formula's weights, every year, permanently. SEEK pays on prior-year average daily attendance (about 91 percent of end-of-year enrollment here); the mostly uncounted at-risk, exceptional-child, language, and transportation weights are worth more than that haircut.", NOTE)
...
v3levers = [
 ...
 ("Fixed positions cut over time (school admin + custodial + library)", 0, 107052.2, 214104.4, "MUNIS FY2026 actuals: school administration $115,397 + custodial $49,655 + library $49,052 = $214,104 (the district's own A.1 prices the same four roles at $209,700, within about 2 percent, and states all current staff are retained in year one; the full-cut leg is an attrition end state)"),
 ...
 ("SEEK add-ons lost per leaver", 0, 500, 1000, "The at-risk weight alone carries the central leg: 15% of base per free-lunch child, about $500 per average student at a ~72% FRL school; exceptional-child, language, and prorated transportation weights sit above it, mostly uncounted"),
]
...
put(c, "C47", "-$409,446: the central case itself loses money, about 16 percent of the structural deficit added, not removed", NOTE)
...
put(c, "C48", "-$846,285 a year: the closure loses money", NOTE)
...
put(ex, "A8", "Current student population (SAAR 2025-26 end-of-year count; the 2024-25 count of 128 anchors the capacity and cost comparisons)"); put(ex, "B8", 115, BLUE, NUM)
...
put(ex, "A18", "Estimate", BOLDW, fill=HDR); put(ex, "B18", "Leave share", BOLDW, fill=HDR); put(ex, "C18", "Of today's 115", BOLDW, fill=HDR); put(ex, "D18", "Per year, full feeder", BOLDW, fill=HDR); put(ex, "E18", "$/yr (feeder)", BOLDW, fill=HDR); put(ex, "F18", "Basis", BOLDW, fill=HDR)
...
put(rd, "A98", "The same discipline applies to the closure direction: 115 arriving students trigger the same class caps at the receiving schools, adding sections in several grades, which is exactly why the closure's net effect runs minus $846,285 to minus $9,860 with a weighted median of minus $427,087, and not the school's $2.5M gross cost.", NOTE, wrap=True)
...
put(al, f"A{tot+8}", "Closure net saving (LEGACY single-point base case, superseded by the grid median of -$427,087 at Closure_Model row 50)"); put(al, f"B{tot+8}", "=Closure_Model!B20", GRN, CUR)
...
put(al, f"A{tot+18}", "Plan levers at the website defaults: 275 of 550 leakage students recovered (275 x $4,226 = $1,162,150) + costs at the $760K low end. Recovered students are priced at supplies-only absorption; each new section a lumpy grade forces costs about $60,000 to $85,000 against this line"); put(al, f"B{tot+18}", 1922150, BLUE, CUR)
...
sav_rows = [
 ("District's own KDE-filed excess cost of NMES vs peer elementaries", 121220, "Above the closure model's central case, which is itself negative (-$409,446); the weighted median loses $427,087"),
 ("Closure model median (v5.0 weighted grid)", -427087, "Closure_Model tab, 972-combination grid; the median scenario loses money"),
 ("Closure model best case (favorable tail)", -9860, "Closure_Model tab B49"),
...
put(d, f"F{r}", "Trend case balances with about $283K to spare; the conservative case is still about $1.13M short, so its total below leans on equalization and existing restricted capacity only", NOTE, wrap=True)
room_r = r; r += 1
...
r += 1
put(d, f"A{r}", "Roughly $21.2 million in the conservative case and $24.9 million on the FY2026 trend, with the budget balanced", NOTE); r += 1
...
put(rw, "A7", "Closure only (central case, a -$409,446 yearly loss, from FY2027)")
put(rw, "B7", "=GF_Summary!D9-GF_Summary!$D$16", BLK, CUR)
for col, prev in zip("CDE", "BCD"):
    put(rw, f"{col}7", f"={prev}7-GF_Summary!$D$16+Closure_Model!$B$47", BLK, CUR)
put(rw, "A9", "Closure range check (v5.0): even the -$9,860 best case drains reserves faster than status quo, the -$427,087 weighted median much faster; every weighted scenario loses money.", NOTE, wrap=True)
...
put(sc, "B6", -427087, BLUE, CUR)
put(sc, "C6", "=Runway!E7", BLK, CUR)
put(sc, "D6", "Unpublished", NOTE)
put(sc, "E6", "Closure vote; the weighted median LOSES $427,087 a year (B6 is the grid median; the legacy single-point Closure_Model!B20 is superseded); the grid central case itself loses $409,446, and even the best-case tail still loses $9,860 a year; longer rides; measured enrollment-loss risk", NOTE)
...
put(sc, "A9", "Reading: the question before the board is not closure versus no closure. It is which complete plan produces the best verified five-year result. "
              "Plan 2 drains reserves faster than doing nothing (Runway rows 5-9); Plan 3 restores balance while keeping every school open. The rebalance-and-grow scenario "
...
put(kc, "A34", "Plan requirement, low / high ($800K-$1M over the 115 students displaced, the 2025-26 end-of-year count)")
put(kc, "B34", "=800000/Exodus_Model!B8", BLK, CUR, bold=True); put(kc, "C34", "=1000000/Exodus_Model!B8", BLK, CUR, bold=True)
put(kc, "A35", "This report's own model, per displaced student: median / central / best case")
put(kc, "B35", "=-427087/Exodus_Model!B8", BLK, CUR); put(kc, "C35", "=Closure_Model!B47/Exodus_Model!B8", BLK, CUR); put(kc, "D35", "=Closure_Model!B49/Exodus_Model!B8", BLK, CUR)
...
put(kc, "A44", "BRACKET CHECK (v3.9): the record's plausible median vs this model's independent bottom-up median per displaced student. These bracket rather than coincide and measure different things: the record credits a district's whole budget change to its closure (an upper bound by construction), the model prices only the levers a closure moves. Before the v3.9 fixed-cost rebuild the model read $713 and the two nearly coincided. Both remain far below the plan's $6,957-$8,696 requirement. The per-student column recomputes from inputs rounded to $1,000s and can differ from the report's exact CSV figures by up to $3 (Webster $3,525, Adair $6,935)."); put(kc, "B44", 818, BLUE, CUR); put(kc, "C44", "=Closure_Model!B47/Exodus_Model!B8", BLK, CUR, bold=True)
...
put(th, "G114", "About -2.5 cents, roughly -$415,000 a year. INFERENCE, NOT A DOCUMENT: KDE does not publish the year-by-year rate-type split, and a facilities nickel is an equivalent rate restated annually against the whole property base, so it drifts. The certified split by year is requested in Appendix B. What is not in doubt: the levy that rose in 2023 was restricted building money that cannot lawfully pay a teacher", NOTE, wrap=True)
