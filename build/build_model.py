...
put(rd, "A127", "Net nonresident import, Bourbon Co district (436 enrolled in vs 247 residents out, Paris flows included)"); put(rd, "B127", 189, BLUE, NUM)
...
put(al, f"A{tot+17}", "THE TRANSFORMATIVE CHECK (v4.4; backs the site plan card and the growth-path section)", SEC)
put(al, f"A{tot+18}", "Plan at LOW ends: enrollment $1.11M + costs $760K"); put(al, f"B{tot+18}", 1870000, BLUE, CUR)
put(al, f"A{tot+19}", "Full 2018 rate restore (Tax_History basis)"); put(al, f"B{tot+19}", 1699479, BLUE, CUR)
put(al, f"A{tot+20}", "Structural gap before transfers"); put(al, f"B{tot+20}", "=Assumptions!B24-Assumptions!B21", GRN, CUR)
put(al, f"A{tot+21}", "Recurring surplus after the gap"); put(al, f"B{tot+21}", f"=B{tot+18}+B{tot+19}-B{tot+20}", BLK, CUR, bold=True)
...
put(al, f"A{tot+24}", "New bonds supported (4.5%, 20 years; same basis as Debt_Service)")
put(al, f"B{tot+24}", f"=B{tot+23}*(1-1.045^-20)/0.045", BLK, CUR, bold=True)
put(al, f"A{tot+25}", "Restricted capacity freed when the sweep ends (Debt_Service)"); put(al, f"B{tot+25}", 17600000, BLUE, CUR)
put(al, f"A{tot+26}", "Building capacity, together"); put(al, f"B{tot+26}", f"=B{tot+24}+B{tot+25}", BLK, CUR, bold=True)
put(al, f"A{tot+27}", "Reading: at the plan's LOW ends with the full restore, the district funds a 5 percent certified raise AND about $23 million of building capacity for new schools and renovations, with every school open. Requires the full 8.9-cent restore and hitting the low-end lever estimates.", NOTE, wrap=True)
put(al, f"A{tot+28}", "Top end: levers high ($3.32M enrollment + $1.32M costs) + full restore = $6.34M vs the $2.65M gap, a $3.69M surplus; a 10 percent certified raise ($1.01M) leaves $2.67M for debt service = about $35M of bonds, roughly $52M of building capacity with the freed restricted stream.", NOTE, wrap=True)
...
put(kc, "B35", "=-21971/Assumptions!B11", BLK, CUR); put(kc, "C35", "=Closure_Model!B47/Assumptions!B11", BLK, CUR); put(kc, "D35", "=Closure_Model!B49/Assumptions!B11", BLK, CUR)
