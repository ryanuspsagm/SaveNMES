...
put(df, "F34", "= $127,039 kept, plus half the fixed overhead positions ($107,052, Closure_Model C40) and three teachers at $54,479.40, minus $63,000 of busing, minus the statistical median of 167 missing students at $5,136 less the $400 supplies credit: the published -$456,383, the 59th percentile of the 3,888-scenario grid; the weighted median is a $523,830 loss (Closure_Model row 50)", NOTE, wrap=True)
...
put(c, "A20", "NET RECURRING GENERAL FUND SAVING (LEGACY single-point scenario on superseded inputs; the published model is the 3,888-scenario grid at row 37, median a $523,830 LOSS)", bold=True)
...
put(c, "A37", "V5.0 TWO-TAILED SENSITIVITY: SEVEN LEVERS, 3,888 COMBINATIONS (backs Figure 5; THIS GRID, not the legacy single-point rows above, is the published closure model)", SEC)
...
put(rd, "A98", "The same discipline applies to the closure direction: 128 arriving students trigger the same class caps at the receiving schools, adding sections in several grades, which is exactly why the closure's net effect runs minus $1,247,265 to plus $171,118 with a weighted median of minus $523,830, and not the school's $2.5M gross cost.", NOTE, wrap=True)
...
put(al, f"A{tot+8}", "Closure net saving (LEGACY single-point base case, superseded by the grid median of -$523,830 at Closure_Model row 50)"); put(al, f"B{tot+8}", "=Closure_Model!B20", GRN, CUR)
...
sav_rows = [
 ("District's own KDE-filed excess cost of NMES vs peer elementaries", 121220, "Above the closure model's central case, which is itself negative (-$463,042); the weighted median loses $523,830"),
 ("Closure model median (v5.0 weighted grid)", -523830, "Closure_Model tab, 3,888-combination grid; the median scenario loses money"),
...
put(rw, "A9", "Closure range check (v5.0): at the +$171,118 best case FY2029 holds a modest cushion; at the -$523,830 weighted median the reserves go MUCH faster than status quo; 99 percent of weighted scenarios lose money and drain reserves faster than doing nothing.", NOTE, wrap=True)
...
put(kc, "B35", "=-523830/Assumptions!B11", BLK, CUR); put(kc, "C35", "=Closure_Model!B47/Assumptions!B11", BLK, CUR); put(kc, "D35", "=Closure_Model!B49/Assumptions!B11", BLK, CUR)
