...
put(al, f"A{tot+17}", "THE TRANSFORMATIVE CHECK (corrected v4.5: enrollment at the model's own Move 2 band; capacity anchored on the district advisor's June 2026 presentation)", SEC)
put(al, f"A{tot+18}", "Plan at TOP ends: enrollment $530K (Move 2 high) + costs $1.30M"); put(al, f"B{tot+18}", 1830000, BLUE, CUR)
put(al, f"C{tot+18}", "CORRECTED: an earlier release used $1.11M-$3.33M for the enrollment lever; the model's own Move 2 rows price it at $260K-$530K and that band now governs. At the LOW ends with the full restore the plan clears the gap with about $71K to spare and funds no raise.", NOTE)
...
put(al, f"A{tot+21}", "Recurring surplus after the gap (top ends + full restore)"); put(al, f"B{tot+21}", f"=B{tot+18}+B{tot+19}-B{tot+20}", BLK, CUR, bold=True)
