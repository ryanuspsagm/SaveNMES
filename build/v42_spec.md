...
5. The closure model, rebuilt on the new levers (3,888 scenarios; leavers priced at
   ...
   - students lost 0-100% (grid values 0/10/20/30/40/50% = 0/13/26/38/51/64 of
     128; slider full range)
   - property-value loss 0-10% of est. zone base ($0-95K; PVA records ask pending);
     kept as real foregone revenue: the board's rate practice does not raise rates
     to compensate for zone valuation losses, so a smaller base is lost revenue
     capacity, and the equity hit lands on zone families either way
...
to Dr. Ryan Bradley's questions; scanned, read page by page):
  ...
  MODEL IMPACT (updated after the top-end and bookend challenges): the staffing
  levers were re-based on this document, then both bookends were inspected.
  (a) Teachers cut run 0/1/2/3 - A.1 prices 2, but Appendix B's own classroom
  ...
  Two exhibit material for the v4.2 build.
